...
SAM_OUT_DIR = REPO_ROOT / "data" / "sam_preprocessed"
...
def run_sam_preprocess(
    ...
    if not checkpoint:
        return "", "", "SAM 모델/체크포인트 경로가 필요합니다.", gr.update(), gr.update()
    ...
    out_root_path = Path(out_root or SAM_OUT_DIR)
    cmd = [
        sys.executable,
        "-u",
        str(REPO_ROOT / "scripts" / "sam_make_masks.py"),
        "--defect-dir",
        defect_dir,
        "--out-root",
        str(out_root_path),
    ...
    with gr.Blocks(title="Defect Synthesis", theme=_muted_theme()) as demo:
        ...
        with gr.Tab("Preprocess"):
            with gr.Row():
                with gr.Column(scale=1):
                    with gr.Row():
                        sam_defect_dir = gr.Textbox(label="결함 이미지 폴더", scale=4)
                        sam_defect_btn = gr.Button("📁", scale=1, min_width=40)
                    with gr.Row():
                        sam_labels_dir = gr.Textbox(label="BB 라벨 폴더 (선택, YOLO txt 기본)", scale=4)
                        sam_labels_btn = gr.Button("📁", scale=1, min_width=40)
                    with gr.Row():
                        sam_out_root = gr.Textbox(
                            label="출력 dataset root",
                            value=str(SAM_OUT_DIR),
                            scale=4,
                        )
                        sam_out_btn = gr.Button("📁", scale=1, min_width=40)
                    with gr.Row():
                        sam_backend = gr.Dropdown(
                            ["ultralytics", "meta"],
                            value="ultralytics",
                            label="SAM backend",
                        )
                        sam_device = gr.Dropdown(["cpu", "cuda"], value="cpu", label="Device")
                    with gr.Row():
                        sam_checkpoint = gr.Textbox(
                            label="SAM 모델/체크포인트",
                            placeholder="e.g. sam_b.pt 또는 /path/to/sam_vit_b.pth",
                            scale=4,
                        )
                        sam_checkpoint_btn = gr.Button("📄", scale=1, min_width=40)
                    with gr.Row():
                        sam_model_type = gr.Dropdown(
                            ["vit_b", "vit_l", "vit_h"],
                            value="vit_b",
                            label="Meta SAM model type",
                        )
                        sam_box_format = gr.Dropdown(
                            ["yolo", "xyxy", "coco"],
                            value="yolo",
                            label="BB format",
                        )
                    with gr.Row():
                        sam_auto = gr.Checkbox(value=False, label="BB 없으면 Auto mask 사용")
                        sam_limit = gr.Number(value=0, precision=0, label="처리 개수 제한 (0=전체)")
                    sam_run_btn = gr.Button("Create Masks", variant="primary")
                with gr.Column(scale=1):
                    sam_b_out = gr.Textbox(label="생성된 B 폴더", interactive=False)
                    sam_mask_out = gr.Textbox(label="생성된 Mask 폴더", interactive=False)
                    sam_log = gr.Textbox(
            ...
            sam_defect_btn.click(pick_dir, [sam_defect_dir], [sam_defect_dir])
            sam_labels_btn.click(pick_dir, [sam_labels_dir], [sam_labels_dir])
            sam_out_btn.click(pick_dir, [sam_out_root], [sam_out_root])
            sam_checkpoint_btn.click(pick_path, [sam_checkpoint], [sam_checkpoint])
        ...
        with gr.Tab("Train"):
            ...
            sam_run_btn.click(
                ...
                outputs=[sam_b_out, sam_mask_out, sam_log, b_in, m_in],
            )
