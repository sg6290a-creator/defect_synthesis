...
class CycleGAN_Mask(BaseModel):
    ...
    def compute_loss_and_update(self, inputs):
        ...
        # fake_mask_B is [B, H, W]; add channel dim for broadcasting against
        # [B, 3, H, W]. Without this, batch_size > 1 breaks the multiplication
        # because batch_size lands in the channel slot.
        fake_mask_B_4d = fake_mask_B.unsqueeze(1)
        input_background_loss = fake_image_B * fake_mask_B_4d
        target_background_loss = real_image_A * fake_mask_B_4d
    ...
    def save_training_progress(self, inputs, mean, std, epoch):
        ...
        plt.figure()
        # Visualize the first sample of the batch (robust to batch_size > 1).
        fake_mask_B = (fake_mask_B[0].detach().cpu().numpy()*255).astype(np.uint8)
        B_mask_real = (B_mask_real[0].detach().cpu().numpy()*255).astype(np.uint8)
