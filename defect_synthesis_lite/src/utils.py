...
def normalize_images_diff(output, mean, std):
    ...
    # Transform [0, 1] -> [0, 255] (use first sample of the batch for visualization)
    real_image_A = np.moveaxis(real_image_A[0].cpu().numpy()*255, 0, -1).astype(np.uint8)
    real_image_B = np.moveaxis(real_image_B[0].cpu().numpy()*255, 0, -1).astype(np.uint8)
    fake_image_A = np.moveaxis(fake_image_A[0].cpu().numpy()*255, 0, -1).astype(np.uint8)
    fake_image_B = np.moveaxis(fake_image_B[0].cpu().numpy()*255, 0, -1).astype(np.uint8)
